  ...
  return (
      ...
      action={async (data: FormData) =>
        createSite(data).then((res: any) => {
          if (res.error) {
            ...
            router.push(`/site/${id}`);
            modal?.hide();
            toast.success(`Création du site réussie!`);
          }
        })
      }
    ...
    >
      <div className="relative flex flex-col space-y-4 p-5 md:p-10">
        <h2 className="font-cal text-2xl dark:text-white">Créer un nouveau site</h2>
        ...
        <div className="flex flex-col space-y-2">
          ...
          <textarea
            name="description"
            placeholder="Courte description du site pour les moteurs de recherche..."
...
<div className=" text-sm italic text-stone-500">
  <br/>
  Vous pouvez changer ces informations à tout moment dans vos paramètres de site.
</div>
...
function CreateSiteFormButton() {
  ...
  return (
    <button
      ...
      disabled={pending}
    >
      {pending ? <LoadingDots color="#808080" /> : <p>Créer le Site</p>}
    </button>
  );
}
